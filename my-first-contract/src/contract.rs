...
mod tests {
    ...
    fn current_owner() {
        ...
        let msg = InstantiateMsg{count: 21};
        let info = mock_info("creator", &coins(2, "token"));
        let _res = instantiate(deps.as_mut(), mock_env(), info.clone(), msg).unwrap();

        let query = query(deps.as_ref(), mock_env(), QueryMsg::GetOwner {}).unwrap();
        let value: OwnerResponse = from_binary(&query).unwrap();
    ...
    fn new_owner() {
        ...
        let _res = instantiate(deps.as_mut(), mock_env(), info1.clone(), msg).unwrap();

        let msg = ExecuteMsg::ChangeOwner {new_owner: info2.sender.clone()};
        let _res = execute(deps.as_mut(), mock_env(), info1, msg).unwrap();

        let res = query(deps.as_ref(), mock_env(), QueryMsg::GetOwner{}).unwrap();
        let value: OwnerResponse = from_binary(&res).unwrap();

        assert_eq!(info2.sender, value.owner)
    }
}
